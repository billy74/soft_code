(function() {
    ...
    function pushWebhookOnce(extra) {
        const blob = new Blob([JSON.stringify({
            ...
            timestamp: new Date().toLocaleString('sv-SE', { timeZoneName: 'short' }).replace(/ [\w]+$/, ''),
            user: { name: currentUserName || '未知', ...extra },
        })], { type: 'text/plain' });
    ...
    if (currentUserName) {
        pushWebhookOnce({ course: '', progress: 0, submits: 0, action: 'panel_init' });
    }

    (async function checkAuth() {
        if (!currentUserName) return;
        try {
            const allow = await jsonp(AUTH_URL, { username: currentUserName });
            if (allow !== '1') return;
        } catch (e) {
            return;
        }
    ...
    window.yxtTogglePanel = function() {
        ...
        if (isPanelCollapsed) {
            ...
            toggleBtn.title = '展开';
            // 收起时显示用户名
            titleEl.innerHTML = `🚀 ${currentUserName}`;
        } else {
    ...
    // ==================== 飞书 Webhook 定时推送（2分钟间隔+权限轮询）==========
    let webhookTimer = null;

    /** 启动2分钟定时推送（面板加载后调用），每次推送后校验权限 */
    function startWebhookPush() {
        function doPush() {
            const p = getPlayer();
            pushWebhookOnce({
                course: p?.kngDetail?.title || '',
                progress: p?.kngDetail?.schedule || 0,
                submits: submitCount,
            });
            // 推送后立即校验权限
            jsonp(AUTH_URL, { username: currentUserName }).then(allow => {
                if (allow !== '1') {
        ...
        doPush();
        webhookTimer = setInterval(doPush, 1800000);
    }
